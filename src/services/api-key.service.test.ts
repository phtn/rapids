...
describe('ApiKeyService', () => {
  ...
  describe('create', () => {
    ...
    test('creates an API key with metadata and scopes', async () => {
      ...
      expect(result.record.metadata).toEqual({ userId: '123', tier: 'premium' });
    ...
    test('uses different charsets', async () => {
      const hexResult = await ApiKeyService.create({ prefix: '', length: 32, charset: 'hex' });
      expect(hexResult.key).toMatch(/^[0-9a-f]+$/);

      const upperResult = await ApiKeyService.create({ prefix: '', length: 32, charset: 'alphanumeric_upper' });
      expect(upperResult.key).toMatch(/^[A-Z0-9]+$/);

      const lowerResult = await ApiKeyService.create({ prefix: '', length: 32, charset: 'alphanumeric_lower' });
  ...
  describe('validate', () => {
    ...
    test('rejects an expired API key', async () => {
      const { key, record } = await ApiKeyService.create({ expiresIn: -1 });
  ...
  describe('list', () => {
    ...
    test('filters by active status', async () => {
      const { record: active } = await ApiKeyService.create({ name: 'Active' });
      const { record: revoked } = await ApiKeyService.create({ name: 'Revoked' });
  ...
  describe('updateMetadata', () => {
    test('updates metadata', async () => {
      const { record } = await ApiKeyService.create({ metadata: { tier: 'free' } });
      
      ApiKeyService.updateMetadata(record.id, { tier: 'premium', extra: 'data' });
  ...
  describe('rate limiting', () => {
    test('enforces rate limits', async () => {
      const { key, record } = await ApiKeyService.create({ rateLimit: 3 });
