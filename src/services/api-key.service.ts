 ...
 */
const DEFAULT_CONFIG: Required<Omit<ApiKeyConfig, 'name'>> & { name: string | null } = {
  prefix: 'rapids_',
  length: 32,
  charset: 'base64url',
  expiresIn: null,
...
export const ApiKeyService = {
  ...
  checkRateLimit(keyId: string, limit: number): boolean {
    ...
    db.run(
      'DELETE FROM rate_limit_records WHERE window_start < ?',
      [windowStart - 60000]
    );
